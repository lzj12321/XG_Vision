...
STR_Vision_Result nccMatchWidget::ncc_Match(cv::Mat &srcMat,int model)
{
    ...
    if(srcMat.type()!=CV_8UC1){
        cv::cvtColor(srcMat,srcMat,CV_RGB2GRAY);
    }
    cv::Mat roiMat;
    ...
    if(!name.empty())
    {
        ...
        if(!src.empty())
        {
            ...
            if(sourceMat.type()!=CV_8UC3)
                cv::cvtColor(sourceMat,sourceMat,CV_GRAY2RGB);
...
bool nccMatchWidget::nccGenerateTemplate(cv::Mat&temp,cv::Mat&mask,bool isEnableMask,bool isEnableRotate)
{
    ...
    if(temp.type()!=CV_8UC1){
        cv::cvtColor(temp,t,CV_RGB2GRAY);
    }else{
        t=temp;
    }
    if(mask.type()!=CV_8UC1){
        cv::cvtColor(mask,m,CV_RGB2GRAY);
    }else{
        m=mask;
    }
...
cv::Mat nccMatchWidget::drawResultOnMat()
{
    ...
    if(sourceMat.type()==CV_8UC1)
        cv::cvtColor(sourceMat,resultMat,CV_GRAY2RGB);
    else
    ...
    if(nccmatch.matchStatus)
    {
        sprintf_s(positionText, "Point:(%d,%d)",nccmatch.nccMatchPoint.x+roiX,nccmatch.nccMatchPoint.y+roiY);
    }
    ...
    if(nccmatch.matchStatus)
    {
        sprintf_s(positionText2, "Angle:%d", matchAngle);
    }
    ...
    if(nccmatch.matchStatus)
    {
        sprintf_s(positionText3, "Value::%.3f",matchScore);
...
cv::Mat nccMatchWidget::drawResultOnMat(NCC_Match&nccmatch)
{
    ...
    if(nccmatch.matchStatus)
    {
        sprintf_s(positionText2, "Angle:%d",nccmatch.nccMatchAngle);
    }
    putText(resultMat, positionText2, textPosition, 2, 1.5, CV_RGB(255, 200, 100),2);
    ...
    if(nccmatch.matchStatus)
    {
        sprintf_s(positionText3, "Value::%.3f",nccmatch.nccMatchScore);
        putText(resultMat, positionText3, textPosition, 2, 1.5, CV_RGB(255, 200, 100),2);
    }
    return resultMat;
}
...
void nccMatchWidget::imgLoad(int model)
{
    if(modelNum==0)return;
    if(model==-1)return;
    templateMat=cv::imread(paramPath+std::string("/")+vecModelName[model].toStdString()+std::string("temp.bmp"),CV_LOAD_IMAGE_COLOR);
    temporyTempMat=templateMat.clone();

    tempMaskShowMat=cv::imread(paramPath+std::string("/")+vecModelName[model].toStdString()+std::string("tempMask.bmp"),CV_LOAD_IMAGE_COLOR);
    temporyTempMaskShowMat=tempMaskShowMat.clone();

    maskMat=cv::imread(paramPath+std::string("/")+vecModelName[model].toStdString()+std::string("mask.bmp"),CV_LOAD_IMAGE_GRAYSCALE);
...
void nccMatchWidget::nccInitialize(int model)
{
    ...
    if(templateMat.type()!=CV_8UC1){
        cv::cvtColor(templateMat,t,CV_RGB2GRAY);
    }else
        t=templateMat;
    if(maskMat.type()!=CV_8UC1){
        cv::cvtColor(maskMat,m,CV_RGB2GRAY);
    }else
        m=maskMat;
...
void nccMatchWidget::nccWidgetIni(cv::Mat &t)
{
    ...
    if(t.type()!=CV_8UC3)
        cv::cvtColor(t,sourceMat,CV_GRAY2RGB);
    else
        sourceMat=t.clone();
    if(!sourceMat.empty()){
        cv::Mat s=sourceMat.clone();
